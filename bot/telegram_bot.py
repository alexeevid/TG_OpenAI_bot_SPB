...
async def on_voice(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if _t_lo.startswith("веб поиск "):
            ...
            try:
                from bot.web_search import web_search_digest, sources_footer
                answer, sources = await web_search_digest(text[10:].strip(), max_results=6, openai_api_key=settings.openai_api_key)
                footer = ("\n\nИсточники:\n" + sources_footer(sources)) if sources else ""
                await _send_long(m, (answer or "Готово.") + footer)
                if sources:
                    from urllib.parse import urlparse
...
async def on_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    _q_lo = q.lower().strip()
    if _q_lo.startswith("нарисуй "):
        from bot.openai_helper import generate_image_bytes
        content, final_prompt = await generate_image_bytes(q[7:].strip())
        await m.reply_photo(photo=content, caption=f"🖼️ Сгенерировано DALL·E 3\nPrompt → {final_prompt}")
        return
    if _q_lo.startswith("веб поиск "):
        note = await m.reply_text("🔎 Ищу в вебе, подожди пару секунд…")
        try:
            from bot.web_search import web_search_digest, sources_footer
            answer, sources = await web_search_digest(q[10:].strip(), max_results=6, openai_api_key=settings.openai_api_key)
            footer = ("\n\nИсточники:\n" + sources_footer(sources)) if sources else ""
            await _send_long(m, (answer or "Готово.") + footer)
            if sources:
                from urllib.parse import urlparse
...
def _save_msg(db, dialog_id: int, role: str, text: str) -> None:
    # Колонка в БД называется "content", не "text"
    db.execute(
        sa_text("""
            INSERT INTO messages (dialog_id, role, content, created_at)
            VALUES (:d, :r, :c, now())
        """),
        {"d": dialog_id, "r": role, "c": text},
    )
...
async def stats(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        with SessionLocal() as db:
            ...
            if not did:
                return await m.reply_text("Нет активного диалога. Создайте новый через /dialogs → «➕ Новый диалог»")
            ...
            lines = [
                ...
                *doc_lines,
                "",
                "📂 База знаний:", f"• Документов: {db.execute(sa_text(\"SELECT COUNT(*) FROM kb_documents\")).scalar() or 0} | Кусков: {db.execute(sa_text(\"SELECT COUNT(*) FROM kb_chunks\")).scalar() or 0}", f"• Средний размер куска: {db.execute(sa_text(\"SELECT COALESCE(AVG(length(content)),0) FROM kb_chunks\")).scalar() or 0} символов (макс {db.execute(sa_text(\"SELECT COALESCE(MAX(length(content)),0) FROM kb_chunks\")).scalar() or 0} симв.)", "", "📈 Счётчики:",
...
async def kb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        with SessionLocal() as db:
            ...
            if not dlg_id:
                await m.reply_text("Нет активного диалога. Создайте новый через /dialogs → «➕ Новый диалог»")
                return
            ...
            # выводим "подключённые" (первая страница, без пагинации — для надёжного старта)
            rows = db.execute(sa_text("""
                SELECT id, path
                FROM kb_documents
                WHERE is_active
            ...
            for doc_id, path in rows:
                fname = (path or "").split("/")[-1] or path or "—"
                checked = "☑" if doc_id in conn_ids else "☐"
                keyboard.append([InlineKeyboardButton(f"{checked} {fname}",
                                 callback_data=f"kb:toggle:{doc_id}:1:connected")])

            # фильтры / сервисные кнопки
            keyboard.append([
                InlineKeyboardButton("Все", callback_data="kb:list:1:all"),
                InlineKeyboardButton("Подключённые", callback_data="kb:list:1:connected"),
                InlineKeyboardButton("Доступные", callback_data="kb:list:1:available"),
            ])
            ...
            await m.reply_text(
                "Меню БЗ: выберите документы для подключения к активному диалогу.",
...
def _kb_page(db, dlg_id: int, page: int, flt: str):
    conn_ids = {
        r[0] for r in db.execute(sa_text(
            "SELECT document_id FROM dialog_kb_links WHERE dialog_id=:d"
        ), {"d": dlg_id}).all()
    ...
    if flt == "connected":
        if conn_ids:
            ...
            return [], 1, 1, conn_ids
    elif flt == "available" and conn_ids:
        where += " AND NOT (id = ANY(:ids))"
        params["ids"] = list(conn_ids)

    total = (db.execute(sa_text(f"SELECT COUNT(*) FROM kb_documents {where}"), params).scalar() or 0)
    ...
    """), {**params, "off": (page-1)*PAGE_SIZE, "lim": PAGE_SIZE}).all()
    return rows, page, pages, conn_ids

def _kb_markup(rows, conn_ids, page, pages, flt, is_admin: bool):
    kb = []
    for doc_id, path in rows:
        fname = (path or "").split("/")[-1] or path or "—"
        checked = "☑" if doc_id in conn_ids else "☐"
        kb.append([InlineKeyboardButton(f"{checked} {fname}",
                 callback_data=f"kb:toggle:{doc_id}:{page}:{flt}")])
    # пагинация
    nav = []
    if page > 1:
        nav.append(InlineKeyboardButton("«", callback_data=f"kb:list:{page-1}:{flt}"))
    ...
    # сервис
    kb.append([InlineKeyboardButton("📎 Сбросить все документы", callback_data="kb:reset_all")])
    if is_admin:
        kb.append([InlineKeyboardButton("🔄 Синхронизация", callback_data="kb:sync")])
    kb.append([InlineKeyboardButton("📁 Статус БЗ", callback_data="kb:status")])
...
async def kb_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        with SessionLocal() as db:
            ...
            dlg_id = _get_active_dialog_id_by_uid(db, uid)
            if not dlg_id:
                await q.message.reply_text("Нет активного диалога — создайте новый через /dialogs → «➕ Новый диалог»")
                return

            if data.startswith("kb:list:"):
                _, _, page, flt = data.split(":", 3)
                rows, page, pages, conn_ids = _kb_page(db, dlg_id, int(page), flt)
                markup = _kb_markup(rows, conn_ids, page, pages, flt, _is_admin(tg_id))
                try:
                    await q.edit_message_text("Меню БЗ: выберите документы для подключения к активному диалогу.", reply_markup=markup)
            ...
            if data.startswith("kb:toggle:"):
                ...
                """), {"d": dlg_id, "doc": doc_id}).first()
                if linked:
                    db.execute(sa_text("DELETE FROM dialog_kb_links WHERE dialog_id=:d AND document_id=:doc"),
                               {"d": dlg_id, "doc": doc_id})
                else:
                    db.execute(sa_text("""
                        INSERT INTO dialog_kb_links (dialog_id, document_id)
                        VALUES (:d, :doc) ON CONFLICT DO NOTHING
                    """), {"d": dlg_id, "doc": doc_id})
                db.commit()
...
async def voice_model_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    m = update.effective_message or update.message
    try:
        client = OpenAI(api_key=settings.openai_api_key)
        res = client.models.list()
        ...
        ids = [it.id for it in models]
        top10 = ids[:10]
        buttons = [[InlineKeyboardButton(mid, callback_data=f"vmodel:set:{mid}")] for mid in top10]
        if len(ids) > 10:
            buttons.append([InlineKeyboardButton("Показать ещё", callback_data="vmodel:more:2")])
        await m.reply_text("Выберите модель для ГОЛОСОВЫХ запросов:", reply_markup=InlineKeyboardMarkup(buttons))
    except Exception:
        log.exception("voice_model_menu failed")
        await m.reply_text("⚠ Не удалось получить список моделей.")

async def voice_model_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    q = update.callback_query
    await q.answer()
    try:
        data = q.data or ""
        if data.startswith("vmodel:more:"):
            page = int(data.split(":")[-1])
            ids = context.user_data.get("all_models_sorted") or []
            start = (page-1)*10; end = page*10
            btns = [[InlineKeyboardButton(mid, callback_data=f"vmodel:set:{mid}")] for mid in ids[start:end]]
            if end < len(ids):
                btns.append([InlineKeyboardButton("Показать ещё", callback_data=f"vmodel:more:{page+1}")])
            return await q.message.reply_text("Модели (голос):", reply_markup=InlineKeyboardMarkup(btns))
        if data.startswith("vmodel:set:"):
            model = data.split(":", 2)[-1]
            with SessionLocal() as db:
                uid = _ensure_user(db, q.from_user.id)
                did = _ensure_dialog(db, uid)
                db.execute(sa_text("UPDATE dialogs SET voice_model=:m WHERE id=:d"), {"m": model, "d": did})
                db.commit()
            await q.message.reply_text(f"✅ Установлена голосовая модель: {model}")
            try:
                await q.message.edit_reply_markup(reply_markup=None)
            except Exception:
                pass
    except Exception:
        log.exception("voice_model_cb failed")
        await q.message.reply_text("⚠ Не удалось установить модель.")

async def voice_mode_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    m = update.effective_message or update.message
    try:
        buttons = [
            [InlineKeyboardButton("ceo", callback_data="vmode:set:ceo")],
            [InlineKeyboardButton("expert", callback_data="vmode:set:expert")],
            [InlineKeyboardButton("pro", callback_data="vmode:set:pro")],
            [InlineKeyboardButton("user", callback_data="vmode:set:user")],
        ]
        await m.reply_text("Выберите стиль для ГОЛОСОВЫХ запросов:", reply_markup=InlineKeyboardMarkup(buttons))
    except Exception:
        log.exception("voice_mode_menu failed")
        await m.reply_text("⚠ Не удалось открыть меню стилей.")
    ...
    try:
        ...
        if data.startswith("vmode:set:"):
            ...
            with SessionLocal() as db:
                uid = _ensure_user(db, q.from_user.id)
                did = _ensure_dialog(db, uid)
                db.execute(sa_text("UPDATE dialogs SET voice_style=:s WHERE id=:d"), {"s": style, "d": did})
                db.commit()
            sample = _STYLE_EXAMPLES.get(style, "")
            await q.message.reply_text(f"✅ Установлен голосовой стиль: {style}\nПример: {sample}")
            try:
                await q.message.edit_reply_markup(reply_markup=None)
            except Exception:
                pass
    except Exception:
        log.exception("voice_mode_cb failed")
        await q.message.reply_text("⚠ Не удалось установить стиль.")
...
def build_app() -> Application:
    # JSON-логи: уровень берём из settings.log_level, иначе INFO
