...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    with get_session() as s:
        ...
        if dbu is None:
            ...
            s.add(dbu); s.commit()
        dlg = s.execute(select(M.Dialog).where(M.Dialog.user_id == dbu.id, M.Dialog.is_deleted == False)
                        .order_by(M.Dialog.created_at.desc())).scalar_one_or_none()
        if dlg is None:
...
async def grant(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not context.args:
        await update.message.reply_text("Использование: /grant <tg_id>"); return
...
async def revoke(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not _user_is_admin(update.effective_user.id):
        await update.message.reply_text("Доступ ограничён."); return
    if not context.args:
        await update.message.reply_text("Использование: /revoke <tg_id>"); return
    tg_id = int(context.args[0])
    with get_session() as s:
        u = s.execute(select(M.User).where(M.User.tg_user_id == tg_id)).scalar_one_or_none()
        if u:
...
async def model_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(f"Текущая модель: {_settings.openai_model} (динамический список можно добавить позже)")

async def mode_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text("Режимы: ceo | expert | pro | user (переключение для диалога — TODO)")

async def handle_text_message(update: Update, context: ContextTypes.DEFAULT_TYPE, override_text: str | None = None):
    user = update.effective_user
    ...
    with get_session() as s:
        ...
        dlg = s.execute(select(M.Dialog).where(M.Dialog.user_id == dbu.id, M.Dialog.is_deleted == False)
                        .order_by(M.Dialog.created_at.desc())).scalar_one_or_none()
        if not dlg:
            dlg = M.Dialog(user_id=dbu.id, title=datetime.now().strftime("%Y-%m-%d | диалог"),
                           style="expert", model=_settings.openai_model)
            s.add(dlg); s.commit()
        try:
