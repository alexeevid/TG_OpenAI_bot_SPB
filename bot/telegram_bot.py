...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    with get_session() as s:
        ...
        if dbu is None:
            from bot.telegram_bot import _user_is_admin
            dbu=M.User(tg_user_id=user.id, is_admin=_user_is_admin(user.id), is_allowed=True, lang='ru'); s.add(dbu); s.commit()
        ...
        if dlg is None:
            dlg=M.Dialog(user_id=dbu.id, title=datetime.now().strftime('%Y-%m-%d | новый диалог'), style='expert', model=_settings.openai_model); s.add(dlg); s.commit()
...
async def handle_text_message(update, context, override_text: str|None=None):
    from bot.openai_helper import chat
    user=update.effective_user
    ...
    if cites:
        answer+='\n\нИсточники: '+ '; '.join(sorted(set(cites))[:5])
