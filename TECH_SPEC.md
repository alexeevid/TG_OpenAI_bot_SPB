TECH_SPEC.md — Техническое задание Telegram-бота с RAG и Базой знаний
Версия ТЗ: 1.0.0
Дата: 2025-07-31
Владелец продукта: Иван Алексеев
Исполнители: команда разработки бота

⚠️ Процесс изменений: Любое изменение логики бота, поведения команд, интерфейсов модулей, конфигурации или зависимостей обязательно предварительно согласуется с владельцем продукта и фиксируется в разделе «Управление изменениями» (с повышением версии ТЗ).
...
Показать полный список доступных моделей OpenAI: gpt-4o, gpt-4.1, gpt-4o-mini, семейства o1*, o3* и др.

Текущая модель помечена ✅.

Недоступные модели (нет прав/доступа) отображаются с 🔒 и недоступны к выбору.

Выбор модели — персональный и диалог-специфичный (не влияет на других пользователей и их диалоги).
...
В подписи к картинке выводить фактический prompt, ушедший в API.
...
Если пользователь — админ: автозапуск синхронизации (Я.Диск → индекс) и вывод итога: added/updated/deleted/unchanged + подсветка новинок.

Отобразить список всех документов БЗ с чекбоксами. Отмеченные — в контексте текущего диалога.

Отдельного переключателя «вкл/выкл БЗ» нет:

Если нет выбранных документов — БЗ де-факто «выключена».

Если есть выбранные — «включена» для текущего диалога.
...
(опц.) режим ретрива: pgvector/файловый.

4. RAG — поведение и приоритеты
Если в диалоге есть выбранные документы:

Делается retrieval (pgvector/файловый/TF-IDF) по запросу пользователя;
...
class KnowledgeBaseIndexer:
    ...
    def sync(self) -> tuple[int, int, int, int]:
        """Возвращает (added, updated, deleted, unchanged). Автосинхронизация для админа в /kb."""

    def list_documents(self) -> list[KBMeta]:
        """Полный список метаданных документов в БЗ (для меню чекбоксов)."""
5.2.3. KB Retriever
python
Копировать
Редактировать
...
class DialogState:
    ...
    style: str = "Pro"
    kb_selected_ids: list[str] = field(default_factory=list)  # выбранные документы
Примечание: если/когда подключим БД для диалогов — структура сохраняется (ORM-модель аналогичная).

6. Команды и поведение
Команды должны быть выставлены через post_init во всех scope: default, private, chat_admins.
...
/stats — статистика (диалог/модель/стиль/БЗ/выбранные документы/режим ретрива).

/kb — меню БЗ:

(админ) автосинхронизация + отчёт + список доков с чекбоксами и подсветкой новинок;

(пользователь) список доков с чекбоксами.

/model — выбор модели (полный список, с 🔒 для недоступных; текущая — ✅).

/mode — стиль ответа (Pro/Expert/User/CEO).

/dialogs — список диалогов (Открыть/🗑/➕ Новый).

/img — генерация изображений (подпись с фактическим prompt).

/web — веб-поиск со списком источников.

Сообщения:

Текст: вызывает OpenAIHelper.chat() (+ kb_context при выбранных доках).

Голос: транскрибация → вывод «🎙️ Вы сказали: …» → chat() с тем же контекстом.
...
CREATE TABLE IF NOT EXISTS kb_chunks (
  ...
  page INTEGER,
  text TEXT,
  embedding VECTOR(1536), -- под используемую размерность (например, text-embedding-3-small)
  score FLOAT DEFAULT 0
);

/* Рекомендуемый индекс для векторного поиска:
...
Опциональные (дефолты в settings.py):

OPENAI_MODEL — дефолтная чат-модель (например, gpt-4o)

OPENAI_TEMPERATURE — дефолт (например, 0.2)
...
YANDEX_DISK_FOLDER — корень БЗ (например, База Знаний)

KB_EMBEDDING_MODEL — например, text-embedding-3-small

KB_MAX_CHUNK_TOKENS — например, 600

KB_TOP_K — например, 6

KB_VECTOR_DB_URL — URL PostgreSQL (нормализовать)

ALLOWED_MODELS_WHITELIST — JSON-массив строк для фильтра меню моделей (опц.)

MODELS_BLACKLIST — JSON-массив строк (опц.)
...
Производительность: RAG-контекст не превышает лимит токенов модели; k чанков по релевантности.
...
/model — полный список (вкл. o1/o3 при наличии доступа), текущая модель с ✅, выбор не влияет на других пользователей и их диалоги.
...
/kb (админ) — автосинхронизация + отчёт; список документов с чекбоксами и подсветкой новинок.
...
RAG — при выбранных документах ответ опирается на БЗ (видны упоминания источников); если ничего не найдено — честно сообщаем.
